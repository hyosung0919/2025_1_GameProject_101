...
public class FruitGame : MonoBehaviour
{
    ...
    public GameObject currentFruit;                 //현재 들고 잇는 과일
    ...
    public Camera mainCamer;                        //카메라 참조(마우스 위치 변환에 필요)
    ...
    void Start()
    {
        mainCamer = Camera.main;                    //메인 카메라 참조 가져 오기
        ...
        if (currentFruit != null)           //현재 생성된 과일이 있을 때만 처리
            {
                ...
                Vector3 worldPosition = mainCamer.ScreenToWorldPoint(mousePosition);        //마우스 위치를 월드 좌표로 변환
