...
public class Fruit : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        Fruit otherFruit = collision.gameObject.GetComponent<Fruit>();      //다른 과일과 충동했는지 확인

        if(otherFruit != null && !otherFruit.hasMered && otherFruit.fruitType == fruitType)  //충동한 것이 과일이고 타일이 같다면 (합쳐지지 않았을 경우)
        {
