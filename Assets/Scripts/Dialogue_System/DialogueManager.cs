...
public class DialogueManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI dialougeText;                        //대화 내용 표시 텍스트 
    ...
    private int currentLineIndex;                                   //현재 몇 번째 대화 중인지 (0 부터 시작)
    ...
    private void CompleteTyping()                               //타이핑 효과를 즉시 완료 하는 함수
    {
        ...
        isTyping=false ;                                        //타이핑 상태 헤제

        if(currentDialogue != null && currentLineIndex < currentDialogue.dialougeLines.Count)
        {
            dialougeText.text = currentDialogue.dialougeLines[currentLineIndex];        //현재 줄의 전체 텍스트를 즉시 표시
        }
    }

    void ShowCurrentLine()                      //현재 대화 줄의 내용을 타이핑 효과와 함께 화면에 표시하는 함수
    {
        if(currentDialogue != null && currentLineIndex < currentDialogue.dialougeLines.Count)       //대화 인덱스가 유효한지 확인
        {
    ...
    public void ShowNextLine()                  //다음 대화 줄로 이동 시키는 함수 (타이핑
    {
        currentLineIndex++;                     //다음 줄로 인덱스 증가

        if(currentLineIndex >= currentDialogue.dialougeLines.Count)     //마지막 대화였는지 확인
        {
            EndDialogue();
        }
        else
    ...
    {
        if (dialogue == null || dialogue.dialougeLines.Count == 0) return;          //대화 데이터가 없거나 대화 내용이 비어있으면 실행 하지 않음
